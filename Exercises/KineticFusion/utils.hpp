...
void SaveVoxelGrid2SurfacePointCloud_ASCII(const std::string &file_name, 
    ...
    outFile << "element vertex " << num_pts << "\n";
    ...
    for (int i = 0; i < voxel_grid_dim_x * voxel_grid_dim_y * voxel_grid_dim_z; i++) {
        if (std::abs(voxel_grid_TSDF[i]) < tsdf_thresh && voxel_grid_weight[i] > weight_thresh) {
            // 计算体素索引
            int z = i / (voxel_grid_dim_x * voxel_grid_dim_y);
            int y = (i % (voxel_grid_dim_x * voxel_grid_dim_y)) / voxel_grid_dim_x;
            int x = i % voxel_grid_dim_x;

            // 转换体素索引到世界坐标系
            float pt_base_x = voxel_grid_origin_x + x * voxel_size;
            float pt_base_y = voxel_grid_origin_y + y * voxel_size;
            float pt_base_z = voxel_grid_origin_z + z * voxel_size;

            // 将坐标以 ASCII 格式写入
            outFile << pt_base_x << " " << pt_base_y << " " << pt_base_z << "\n";
        }
    }

    // 关闭文件
    outFile.close();
    std::cout << "Successfully saved " << num_pts << " points to " << file_name << " in ASCII PLY format." << std::endl;
}
