...
public:
  ...
  double Eval(const Eigen::Vector3d &_x) {
    ...
    return pow(((_x - m_center).dot(_x - m_center)+ m_radius * m_radius - m_a * m_a),2) -
           4 * m_radius * m_radius * ((_x.x()-m_center.x()) * (_x.x()-m_center.x()) + (_x.y()-m_center.y()) * (_x.y()-m_center.y()));
  }
...
public:
  ...
  double Eval(const Eigen::Vector3d &_x) {
    ...
    result +=
        m_coefficents.tail(4).dot(Eigen::Vector4d(_x.x(), _x.y(), _x.z(), 1.0));
    return result;
  }
