...
public:
  ...
  double Eval(const Eigen::Vector3d &_x) {
    // TODO: implement the implicit sphere formula using the member variables
    // m_center and m_radius
...
public:
  ...
  double Eval(const Eigen::Vector3d &_x) {
    // TODO: implement the implicit torus formula using the  variables m_center,
    // m_radius (radius of the ring) and the radius m_a (small radius)

    return ((_x - m_center).norm + m_radius * m_radius - m_a * m_a) -
           4 * m_radius * m_radius * (_x.x() * _x.x() + _x.y() * _x.y());
    ;
  }
...
public:
  Hoppe(const std::string &filenamePC) su {
    m_pointcloud.ReadFromFile(filenamePC);
  }

  double Eval(const Eigen::Vector3d &_x) {
    Eigen::Vector3f x =
        Eigen::Vector3f((float)_x.x(), (float)_x.y(), (float)_x.z());
...
public:
  ...
  double Eval(const Eigen::Vector3d &_x) {
    // TODO: eval the RBF function based on the coefficents stored in
    // m_coefficents (see lecture slides) the first m_numCenters entries contain
    // the coefficients of the kernels (that can be evaluated using EvalBasis())
    // the following parameters are the coeffients for the linear and the
    // constant part
    double result = EvalBasis((m_coefficents.head(m_numCenters).dot(_x)).norm) +
                    m_coefficents.tail(4).dot(_x);
...
#define phi(i, j) EvalBasis((m_funcSamp.m_pos[i] - m_funcSamp.m_pos[j]).norm())
  ...
  void BuildSystem() {
    ...
    for (int i = 0; i < m_numCenters; i++) {
      ...
      m_systemMatrix.col(i).segment(m_numCenters, 4) =
          m_systemMatrix.row(i).segment(m_numCenters, 4);
  ...
  //! The given function samples (at each function sample, we place a basis
  //! function).
  ...
  //! the right hand side of our system of linear equation. Unfortunately,
  //! float-precision is not enough, so we have to use double here.
  VectorXd m_rhs;

  //! the system matrix. Unfortunately, float-precision is not enough, so we
  //! have to use double here
  MatrixXd m_systemMatrix;

  //! store the result of the linear system here. Unfortunately, float-precision
  //! is not enough, so we have to use double here
...
public:
  RBF(const std::string &filenamePC) {
    ...
    for (unsigned int i = 0; i < m_pointcloud.GetPoints().size(); i++) {
      ...
      m_funcSamp.insertSample(pt + n * eps,
                              eps); // off surface point => distance = eps
      eps *= -1;
    }
  ...
  double Eval(const Eigen::Vector3d &_x) {
    // TODO: eval the RBF function based on the coefficents stored in
    // m_coefficents the first m_numCenters entries contain the coefficients of
    // the kernels (that can be evaluated using EvalBasis()) the following
    // parameters are the coeffients for the linear and the constant part the
    // centers of the RBFs are the first m_numCenters sample points (use
    // m_funcSamp.m_pos[i] to access them) hint: Eigen provides a norm()
    // function to compute the l2-norm of a vector (e.g. see macro phi(i,j))
    double result = EvalBasis();

    result =
        // EvalBasis((m_coefficents.head(m_numCenters).dot(_x)).norm) +
        //     m_coefficents.tail(4).dot(_x)
...
#define phi(i, j) EvalBasis((m_funcSamp.m_pos[i] - m_funcSamp.m_pos[j]).norm())
  ...
  void BuildSystem() {
    ...
    // TODO fill the matrix A and the vector b as described in the exercise
    // sheet note that all sample points (both on and off surface points) are
    // stored in m_funcSamp you can access matrix elements using for example
    // A(i,j) for the i-th row and j-th column similar you access the elements
    // of the vector b, e.g. b(i) for the i-th element
    for (int i = 0; i < 2 * m_numCenters; i++) {
      for (int j = 0; j < m_numCenters; j++)
        A(i, j) = phi(i, j);

      A.row(i).segment(m_numCenters, 3) = m_funcSamp.m_pos[i];
      A.row(i)(m_numCenters + 3) = 1;
    ...
    m_systemMatrix.diagonal() +=
        lambda * lambda * VectorXd::Ones(m_numCenters + 4);
  }
